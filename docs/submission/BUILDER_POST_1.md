...
The proposed AWS agent has a limited job: help organise the evidence, use calculation tools and prepare proposed work. Its Strands integration is tested locally with controlled model inputs; actual Bedrock inference and AWS deployment remain pending. We chose a separation between model output and confirmed facts. The administrator decides consequential changes, and a shared report contains a deliberate snapshot.
...
**Publication evidence to add:** actual portfolio/decision screenshot, verified release link, a short account of the observed AWS run and any failed or unfinished behavior. Link the later architecture and evaluation posts once they are public.
