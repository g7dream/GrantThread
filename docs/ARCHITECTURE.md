...
This diagram describes the AWS stack deployed on 14 September 2026 and the shared local implementation. `grantthread-demo` reached `CREATE_COMPLETE` in `eu-north-1`; a genuine queued Strands/Nova Lite reconciliation persisted eight successful tool events and an incomplete report draft before reaching `waiting_input` for human review. The hosted grantee login/reload, financial display and downloaded XLSX values now pass after CORS repairs. A fresh portfolio-wide job separately exhausted its tool budget with one rejected evidence reference and no saved draft. Hosted sharing and a source-linked question/answer also pass. Hosted receipt-PDF values, the selected original download and funder acknowledgement are verified. Repeated judge demonstrations remain open. Backend administrative-invoke tests do not substitute for JWT or browser verification. Exact evidence is in [EVALUATION.md](EVALUATION.md).
...
The web host serves compiled public assets; persistent workers are designed to run on AWS. API and agent worker share one Python domain layer. Bank-file imports and per-grant currency conversion are implemented. Direct bank-account feeds, market exchange-rate feeds, AgentCore, OCR and consolidation into one portfolio currency are outside this release. The AWS backend, genuine reconciliation execution and actual hosted grantee login/reload/XLSX download are verified. Hosted sharing and a source-linked question/answer also pass; hosted receipt-PDF values, the selected original download and funder acknowledgement are verified. The later portfolio-wide job demonstrated a tool-budget stop without a draft. A separate set of 34 administrative Lambda checks exercised real S3 financial XLSX/PDF imports, receipt conversion, payment matching, correction from CAD 25 to CAD 20 and exports. Those checks do not establish browser authentication or genuine model-based bank extraction. Public demonstrations use fictional records only. Sensitive documents require a separately operated, access-controlled environment with validated retention and operational controls.
