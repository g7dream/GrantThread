...
6. For the invited-account route, create the three synthetic judge accounts in the Cognito console and complete their initial password challenge through managed login. Keep passwords in a private credential handoff. Account creation/invitations are deliberate operator actions; the script below sends no emails. Alternatively, deploy `PublicDemoSignup=true` (enabled for the current public demo) to offer native email/password signup and a separate editable three-grant copy per visitor with Grantee/Funder switching and Restore. See [PUBLIC_DEMO.md](../docs/PUBLIC_DEMO.md). Existing invited memberships keep their original scope. Real signup email delivery/code confirmation remain unverified; the personal-demo OAuth/start/role/Restore browser journey passes.
...
The canonical invited-workspace cloud reset is operator-only: `python infra/manage_demo.py --stack STACK --region REGION --reset-synthetic --confirm-stack STACK`. It replaces the synthetic organisation aggregates and preserves memberships. This differs from the authenticated visitor's **Restore demo**, which replaces only that visitor's copy and preserves login/job history. Old private S3 objects remain in both cases and require deliberate later retention cleanup. Never use the operator reset against real records.
