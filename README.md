...
Source is published at [g7dream/GrantThread](https://github.com/g7dream/GrantThread) under the MIT licence. Public access, default branch `master`, README and licence were independently verified.

## Current release status

**14 September 2026 — public editable demo update:** the current **280 local tests** pass (190 backend, 52 scripts, 38 frontend), plus the production build. AWS signup/health/CORS settings and all 16 hosted encoding/cache checks pass. The browser loads `index-C8T0tDcQ.js`; **18 actual browser checks** cover OAuth, explicit personal-copy creation, editing, report sharing, both roles, a downloaded original, a resolved conversation and Restore with login/history preserved. **55 separate administrative cloud checks** cover two-visitor isolation and restoration boundaries. Test accounts were confirmed administratively: real signup email delivery and code confirmation remain unverified. The latest AI runs stopped at a budget or failed under model throttling; automatic completion is not guaranteed.

## Public editable demo

At [GrantThread](https://timeillusion.com/grantthread/), choose **Create demo account**, use an email address and a GrantThread password, then complete email verification. The secure sign-in page is hosted by AWS; you do not need an AWS account. A Gmail address works as an email address. Google single sign-on is not configured.

Choose **Create my editable demo** to create your own starting copy: Bright Path Lab's three grants and the Northstar funder view. Edit fictional records, share a report, switch between **Grantee** and **Funder**, and use **Restore demo** for another take. Restore checks the current version and waits for active jobs; it preserves your login and another visitor's records. It replaces your scenario, not the operator's invited demo. It retains agent history and does not permanently delete S3 objects or reset the daily agent allowance. Use fictional uploads only.

See [the public-demo guide](docs/PUBLIC_DEMO.md) for signup, recording steps and restore limits, and [the evaluation record](docs/EVALUATION.md) for each verification method and remaining gates. Existing invited accounts keep their assigned workspaces and do not receive the public-demo role or restore controls. The earlier invited-user results below are separate historical evidence.

## Earlier invited-account verification — 14 September
...
The latest completed local suites passed **280 tests: 190 backend, 52 release/build-script and 38 frontend API/auth tests**, plus the production frontend build. The targeted checks cover money, currency, concurrent/stale actions, tenant scope, upload bounds, source versions, batch atomicity, shared projections, PDF text, clarification lifecycle, queue fencing, tool budgets and model timeout. The scripted benchmark runs three isolated **local service-layer** journeys; it excludes human review and does not establish time savings. Additional repeated hosted rehearsals are recommended for confidence; they are not a prerequisite to using the demo.
