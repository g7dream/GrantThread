...
| D4.1 | Targeted gates and synthetic benchmark | Local complete; release gates open | 116 checks and 3 isolated service journeys passed. G1 and three deployed journeys remain unverified. Human timing not measured. |
...
| F2 | Excel ledger and report-template import | Complete locally | Private originals, mapped previews, source rows, effective date formats and preserved template styles/formulas. Supplied layouts checked in memory; separate income table flagged. |
| F3 | Bank reconciliation and AI proposals | Implemented; genuine model run open | Text statement parsing, scoped Strands extraction, duplicate/overlap prevention, payment matching, credits and incomplete-source review. Live Bedrock still unavailable. |
